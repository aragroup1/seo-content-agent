...
# Cleaners: remove shipping phrases, (M) etc., and Excel artifacts like #VALUE!
SHIPPING_PHRASES = [
    "Large Letter Rate",
    "Big Parcel Rate",
    "Small Parcel Rate",
    ...
    async def get_all_products(self, limit=250):
        ...
        while True:
            try:
                ...
                since_id = batch[-1]["id"]
                page += 1
                if len(products) >= 500000:  # hard safety
                    break
...
# ---------------------------------------------------------
# AI Service (JSON-compliant prompts)
# ---------------------------------------------------------
...
- "title": a descriptive page title, aim ~60–70 characters, no special characters
...
# ---------------------------------------------------------
# Bulk scan (used by API and scheduler)
# ---------------------------------------------------------
...
# FastAPI app
# ---------------------------------------------------------
app = FastAPI(title="AI SEO Content Agent", version="auto-worker-1.0")
app.add_middleware(
    CORSMiddleware,
